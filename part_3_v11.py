#!/usr/bin/env python3
"""Part (c) variant v11: same as part_3_v9.py (part_3_v4.py's quadratic
autocorr/cross-correlation + BVP peak-interval HRV features, Yeo-Johnson
power transform) but with RidgeCV in place of ElasticNetCV.

Usage:
    python3 part_3_v11.py train.csv test.csv predictions.txt
...
Ridge is closed-form per alpha (no coordinate-descent path over an
l1_ratio x alpha grid), so this is also a much cheaper fit than
part_3_v9.py's ElasticNetCV for the same feature representation.
"""
import sys
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v11.py train.csv test.csv predictions.txt")
        sys.exit(1)
