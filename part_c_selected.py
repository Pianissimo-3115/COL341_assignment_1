...
featureCountCandidates = (200, 300, 400, 600, 800, 1200, 100000)
...
def selectColumns(design, target, blockIdOfColumn):
    splitIndex = int(len(target) * 0.75)
    splitMean = target[:splitIndex].mean()
    columnScale = (1.0 / np.sqrt(np.asarray(blockPenaltyPresets[0], np.float64)))[blockIdOfColumn]
    gram, rhs = normalEquations(design[:splitIndex], (target[:splitIndex] - splitMean).astype(np.float32))
    _, _, rankingWeights = bestPenaltyOnRidgePath(gram, rhs, columnScale, design[splitIndex:], target[splitIndex:], splitMean)
    ranking = np.argsort(-np.abs(np.asarray(rankingWeights, np.float64)))
    bestSelection, bestScore = None, None
    for candidate in featureCountCandidates:
        selection = np.sort(ranking[:min(candidate, design.shape[1])])
        score, _, _ = bestPenaltyOnRidgePath(
            gram[np.ix_(selection, selection)], rhs[selection], columnScale[selection],
            np.ascontiguousarray(design[splitIndex:][:, selection]), target[splitIndex:], splitMean)
        logProgress("  keep %-6d internal NMAE %.4f" % (len(selection), score))
        if bestScore is None or score < bestScore:
            bestScore, bestSelection = score, selection
        if len(selection) == design.shape[1]:
            break
    logProgress("  keeping %d of %d columns (internal NMAE %.4f)"
                % (len(bestSelection), design.shape[1], bestScore))
    return bestSelection


def main():
    if len(sys.argv) != 4:
        sys.stderr.write("usage: part_c_selected.py train.csv test.csv predictions.txt\n")
        return 2
    ...
    selected = selectColumns(trainDesign, trainTarget, blockIdOfColumn)
    trainDesign = np.ascontiguousarray(trainDesign[:, selected])
    blockIdOfColumn = blockIdOfColumn[selected]
    gc.collect()
    logProgress("selected design %s" % (trainDesign.shape,))
    ...
    testDesign = np.ascontiguousarray(testDesign[:, selected])
