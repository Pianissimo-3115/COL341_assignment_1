#!/usr/bin/env python3
"""Part (c) variant v4 (own idea): part_3_v1.py (quadratic corr features)
+ pulse-interval / heart-rate-variability features from simple local-
maxima peak detection on the BVP waveform.

Usage:
    python3 part_3_v4.py train.csv test.csv predictions.txt
...
Rationale: the autocorrelation and cross-correlation features summarize
periodicity indirectly (best-matching lag, correlation-vs-offset curve).
This variant instead detects individual pulse peaks directly (a sample is
a peak if it is strictly greater than both neighbours) and derives:
  - peak count over the 10s window -> a direct beats-per-window rate;
  - mean and std of the inter-peak interval -> implied bpm and a genuine
    heart-rate-variability proxy (coefficient of variation of interval);
these are physiologically distinct from a correlation peak: two windows
can have the same best-lag autocorrelation but very different pulse
regularity, which only inter-peak timing captures. Peak detection itself
is a per-row Python loop (variable number of peaks per window prevents a
clean vectorized reduction), which is fine at typical assignment dataset
sizes but is the most expensive part of feature creation here.
"""
import sys
...
def peak_interval_features(arr, fs, prefix):
    """Local-maxima pulse detection (a sample is a peak if strictly
    greater than both neighbours) -> peak count, mean/std/CV of the
    inter-peak interval, and the interval-implied bpm. Falls back to
    zeros when fewer than two peaks are found in a window.
    """
    n, T = arr.shape
    ...
    for i in range(n):
        ...
        if peak_idx.size >= 2:
            ...
            feats[i, 1] = mean_iv
            feats[i, 2] = std_iv
            feats[i, 3] = std_iv / (mean_iv + 1e-8)
            feats[i, 4] = 60.0 / (mean_iv + 1e-8)
    names = [
        f"{prefix}_peak_count",
        f"{prefix}_peak_mean_iv",
        f"{prefix}_peak_std_iv",
        f"{prefix}_peak_cv_iv",
        f"{prefix}_peak_bpm",
    ]
    return feats, names
...
def build_features(X):
    ...
    pk_feats, pk_names = peak_interval_features(signals["bvp"], BVP_FS, "bvp")
    blocks.append(pk_feats)
    names.extend(pk_names)
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v4.py train.csv test.csv predictions.txt")
        sys.exit(1)
