#!/usr/bin/env python3
"""Part (c) variant v9: part_3_v4.py's feature set (quadratic autocorr/
cross-correlation features + BVP peak-interval HRV features) with a
Yeo-Johnson power transform in place of plain standardization. Model stays
ElasticNetCV, unchanged from part_3_v4.py, so the only variable being
tested here is the outlier-taming transform.

Usage:
    python3 part_3_v9.py train.csv test.csv predictions.txt
...
See part_3_v5.py for the outlier-handling rationale (several created
features -- `*_energy`, `*_std`, `bvp_xcorr_off*` -- are heavy-tailed, and
Yeo-Johnson compresses those tails per feature while folding in the final
zero-mean/unit-variance scaling). Here it is applied to part_3_v4.py's
feature set instead of part_3_v2.py/part_3_v3.py's.
"""
import sys
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v9.py train.csv test.csv predictions.txt")
        sys.exit(1)
