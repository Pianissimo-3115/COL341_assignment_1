...
coarseFactor = 4
coarseBinCount = candidateCount // coarseFactor                   
...
def coarsenBins(rows):
    return rows.reshape(len(rows), coarseBinCount, coarseFactor).sum(2)
...
softmaxTemperatures = (0.12, 0.25, 0.5, 1.0)
...
def buildScalarFeatures(bvp, acc, eda, logRatePrior):
    columns, columnNames, distributionBlocks = [], [], {}
    rowCount = len(bvp)
    surfaces, bvpBandPassed, spectrum, accSpectrum = buildEvidenceSurfaces(bvp, acc, logRatePrior)

    mainScore = combineSurfaces(surfaces, mainScoreWeights)
    mainRateBpm, peakScores, mainScoreEntropy = describeSurface(mainScore, "S", columns, columnNames)
    for index, temperature in enumerate(softmaxTemperatures):
        distributionBlocks["sm%d" % index] = coarsenBins(softmaxRows(mainScore, temperature))
    ...
    distributionBlocks["sm_c"] = coarsenBins(softmaxRows(consensusScore, 0.25))
    ...
    distributionBlocks["specP"] = coarsenBins(spectrum)
    distributionBlocks["specS"] = coarsenBins(normaliseRows(np.sqrt(spectrum)))
    distributionBlocks["ar"] = coarsenBins(softmaxRows(surfaces["lAR"], 1.0))
    distributionBlocks["g25"] = coarsenBins(softmaxRows(surfaces["g25"], 1.0))
    distributionBlocks["accs"] = coarsenBins(accSpectrum)
    ...
    selectedPeakProminence = lookupPerRowBpm(surfaces["lP"], mainRateBpm).astype(np.float32)
    del surfaces

    logPulseAmplitude = addBvpWaveformFeatures(bvp, bvpBandPassed, columns, columnNames)
    logMotionLevel = addMotionFeatures(acc, bvpBandPassed, columns, columnNames)
    addEdaFeatures(eda, columns, columnNames)

    scalarMatrix = np.hstack(columns).astype(np.float32)
    gateValues = {"sel": mainRateBpm, "sel2": consensusRateBpm, "sel3": spectralRateBpm, "pk_g2": consensus2sRateBpm, "pk_P": periodogramRateBpm,
            "pk_AR": autoregressiveRateBpm, "pk_g25": consensus25sRateBpm, "pk_g5": consensus5sRateBpm,
            "margin": (peakScores[:, 0] - peakScores[:, 1]).astype(np.float32),
            "sent": mainScoreEntropy.astype(np.float32), "lamp": logPulseAmplitude, "lacc": logMotionLevel,
            "xstd": rateEstimates.std(1).astype(np.float32), "xagree": estimatorAgreement, "lprom": selectedPeakProminence}
    return scalarMatrix, columnNames, distributionBlocks, gateValues


def buildScalarFeaturesChunked(bvp, acc, eda, logRatePrior, chunkRows=8192, tag=""):
    scalarChunks, distributionChunks, gateChunks, columnNames = [], None, None, None
    rowCount = len(bvp)
    for start in range(0, rowCount, chunkRows):
        stop = min(start + chunkRows, rowCount)
        scalars, chunkColumnNames, distributions, gates = buildScalarFeatures(bvp[start:stop], acc[start:stop], eda[start:stop], logRatePrior)
        scalarChunks.append(scalars)
        if distributionChunks is None:
            columnNames = chunkColumnNames
            distributionChunks = {key: [value] for key, value in distributions.items()}
            gateChunks = {key: [value] for key, value in gates.items()}
        else:
            for key, value in distributions.items():
                distributionChunks[key].append(value)
            for key, value in gates.items():
                gateChunks[key].append(value)
        del scalars, distributions, gates
        gc.collect()
        logProgress("  %s features %d/%d" % (tag, stop, rowCount))
    return (np.concatenate(scalarChunks), columnNames,
            {key: np.concatenate(value) for key, value in distributionChunks.items()},
            {key: np.concatenate(value) for key, value in gateChunks.items()})


bumpsPerScalar = 7 if fastMode else 9
rateBumpCentres = np.arange(40.0, 200.1, 8.0 if fastMode else 6.0)
confidenceGateNames = ("lamp", "xagree", "margin") if fastMode else ("lamp", "xagree", "margin", "lacc", "sent")
gatePairNames = (("lacc", "lamp"), ("lacc", "sent"), ("lamp", "xagree"))
...
def fitGateBumpKnots(gateValues):
    gateKnots = {gateName: np.percentile(gateValues[gateName], np.linspace(5, 95, 5)) for gateName in confidenceGateNames}
    pairKnots = {name: np.percentile(gateValues[name], np.linspace(4, 96, 7)) for pair in gatePairNames for name in pair}
    return gateKnots, pairKnots


def expandRateByConfidence(gateValues, gateKnots, pairKnots):
    columns = []
    for rateName in ("sel", "sel2", "pk_g2"):
        rateBumps = gaussianBumpBasis(gateValues[rateName], rateBumpCentres, 6.0)
        for gateName in confidenceGateNames:
            gateKnotCentres = gateKnots[gateName]
            gateBumps = gaussianBumpBasis(gateValues[gateName], gateKnotCentres, max((gateKnotCentres[-1] - gateKnotCentres[0]) / 4.0, 1e-6))
            columns.append((rateBumps[:, :, None] * gateBumps[:, None, :]).reshape(len(rateBumps), -1))
    for firstName, secondName in gatePairNames:
        firstKnots, secondKnots = pairKnots[firstName], pairKnots[secondName]
        firstBumps = gaussianBumpBasis(gateValues[firstName], firstKnots, max((firstKnots[-1] - firstKnots[0]) / 6.0, 1e-6))
        secondBumps = gaussianBumpBasis(gateValues[secondName], secondKnots, max((secondKnots[-1] - secondKnots[0]) / 6.0, 1e-6))
        columns.append((firstBumps[:, :, None] * secondBumps[:, None, :]).reshape(len(firstBumps), -1))
...
blockPenaltyPresets = ((0.1, 1.0, 10.0, 10.0),
           (1.0, 1.0, 1.0, 1.0),
           (0.03, 0.3, 3.0, 30.0),
...
def main():
    ...
    trainScalars, columnNames, trainDistributions, trainGates = buildScalarFeaturesChunked(bvp, acc, eda, logRatePrior, tag="train")
    del bvp, acc, eda
    gc.collect()
    ...
    testRows = len(bvp)
    testScalars, _, testDistributions, testGates = buildScalarFeaturesChunked(bvp, acc, eda, logRatePrior, tag="test")
    del bvp, acc, eda
    gc.collect()

    knotCentres, knotWidths = fitScalarBumpKnots(trainScalars)
    gateKnots, pairKnots = fitGateBumpKnots(trainGates)
    distributionOrder = list(trainDistributions.keys())

    def buildDesign(scalars, distributions, gates):
        return [scalars, expandScalarsToBumps(scalars, knotCentres, knotWidths), expandRateByConfidence(gates, gateKnots, pairKnots),
                np.hstack([distributions[name] for name in distributionOrder]).astype(np.float32)]

    trainParts = buildDesign(trainScalars, trainDistributions, trainGates)
    del trainScalars, trainDistributions, trainGates
    gc.collect()
    ...
    testParts = buildDesign(testScalars, testDistributions, testGates)
    del testScalars, testDistributions, testGates
    gc.collect()
