#!/usr/bin/env python3
"""Part (c) variant v2: identical features to part_c_better.py, but the
final model is RidgeCV instead of ElasticNetCV.

Usage:
    python3 part_3_v2.py train.csv test.csv predictions.txt
...
fitting a permitted linear model). Ridge regression is an explicitly
permitted final model, and RidgeCV is closed-form per alpha, so this file
also serves as a much cheaper reference point than the L1/L2-mixed
ElasticNetCV search.
"""
import sys
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v2.py train.csv test.csv predictions.txt")
        sys.exit(1)
