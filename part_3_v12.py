#!/usr/bin/env python3
"""Part (c) variant v12 (merge): part_3_v7.py's full feature set (the best
performer so far) plus a compact, non-redundant autocorrelation summary
adapted from part_3_v6.py -- the one genuinely distinct signal that
part_3_v4.py (the second-best performer) had that v7 lacks, stripped of
the 72-feature sliding cross-correlation bank that v4/part_c_better.py
carried alongside it.

Usage:
    python3 part_3_v12.py train.csv test.csv predictions.txt
...
----------------------
Empirically (per local competitive testing across part_c.py..v11): v7 has
performed best, v4 second, and every version built around the wide
sliding-xcorr-offset bank (part_c_better.py, v1, v2, v3, v5, v9, v11)
clustered together in the middle. The working theory is that v7 wins
because its EWMA/recency-contrast features explicitly weight the samples
closest to the prediction time t (the label is measured at the end of the
10s window), and because it never pays the cost of ~72 highly collinear
xcorr-offset columns. v4's edge over the xcorr-bank-only versions is
credited to its autocorrelation-derived heart-rate estimate -- a
genuinely different signal-processing idea (periodicity via self-
similarity) than anything in v7 (peak morphology, zero-crossing rate).

This version keeps every v7 feature family unchanged and adds only:
- `harmonic_autocorr_bank` (from part_3_v6.py): the best-matching lag's
  implied bpm and correlation strength, a guarded second peak (periodicity
  confidence ratio), and a harmonic-sum lag search that reinforces lags
  whose small integer multiples also correlate well (robust to octave
  errors) -- 7 features from a single correlation-matrix pass, not a
  72-offset bank.
- Squared companions of those 7 (part_3_v1.py's insight: a correlation
  coefficient's *magnitude* is what indicates a genuine periodic match,
  regardless of sign/phase, which a plain linear term cannot represent).

The model now has three independent heart-rate estimators feeding it
(peak-morphology bpm, zero-crossing bpm, autocorrelation/harmonic bpm)
plus the full recency/morphology/motion context, and Ridge -- rather than
ElasticNet -- lets it blend all three by shrinkage instead of forcing a
sparse pick among a large collinear bank.
"""
import sys
...
def harmonic_autocorr_bank(arr, fs, hr_min, hr_max, prefix, n_harmonics=N_HARMONICS):
    ...
    a sliding cross-correlation offset bank -- that redundancy is what
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v12.py train.csv test.csv predictions.txt")
        sys.exit(1)
