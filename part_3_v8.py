#!/usr/bin/env python3
"""Part (c) variant v8: part_3_v7.py's feature set (pulse morphology,
exponential recency-weighting, motion/quality indicators -- no
autocorrelation) plus a degree-3 polynomial trend fit of the raw BVP
waveform, with a Yeo-Johnson power transform in place of plain
standardization.

Usage:
    python3 part_3_v8.py train.csv test.csv predictions.txt
...
Two deltas from part_3_v7.py
-----------------------------
- Polynomial trend (bvp): a degree-3 polynomial fit to the whole 10s
  waveform (constant term dropped -- redundant with the existing mean
  feature) captures broad within-window shape that neither the block-wise
  trend slopes nor the EWMA momentum term directly express. Fit for every
  row at once via a single shared least-squares matrix (numpy.linalg.inv
  only, no pinv, no per-row Python loop) -- explicitly suggested as a
  useful BVP feature family in the assignment statement.
- Yeo-Johnson power transform (all features) instead of StandardScaler:
  several features here are heavy-tailed by construction -- `*_energy` is
  a sum of squares, `*_jerk_energy` and `*_iv_std` can spike on a single
  noisy/high-motion window, and the EWMA "momentum" and exponential-trend
  "rate" terms are unbounded. A single such outlier row can dominate a
  ridge penalty's geometry. Yeo-Johnson (signed generalization of Box-Cox)
  compresses these tails per feature and folds in the final zero-mean/
  unit-variance scaling (`standardize=True`), fit on training features
  only, matching the "any learned preprocessing must be fit using training
  data only" rule.
"""
import sys
...
def build_features(X):
    ...
    f, n = poly_trend_features(bvp, BVP_POLY_DEGREE, "bvp"); blocks.append(f); names.extend(n)
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v8.py train.csv test.csv predictions.txt")
        sys.exit(1)
    ...
    Z_train_pt = power.fit_transform(Z_train)
    Z_test_pt = power.transform(Z_test)

    model = RidgeCV(alphas=np.logspace(-3, 4, 40), cv=5)
    model.fit(Z_train_pt, y_train)

    preds = model.predict(Z_test_pt)
