#!/usr/bin/env python3
"""Part (c) variant v5: part_c_better.py + a Yeo-Johnson power transform to
tame outliers before the linear model sees the features.

Usage:
    python3 part_3_v5.py train.csv test.csv predictions.txt
...
Outlier handling
-----------------
Several of the created features are outlier-prone: `*_energy` is a sum of
squares (heavy right tail whenever a window has a burst of motion or a
high-amplitude pulse), `*_std`/`*_diff_std` inherit the same skew, and the
sliding-window xcorr features (bvp_xcorr_off*) can spike toward +-1 or blow
up when a window's own energy is near zero (see the 1e-8 floor in
sliding_xcorr_features). A single such window can dominate an L1/L2
penalty's geometry and distort the fitted coefficients for everyone else.

Box-Cox is the classical fix (find a power lambda, per feature, that makes
the transformed values closest to Gaussian, compressing the tail), but it
requires strictly positive inputs -- not true here (BVP/acc/diff/slope
values and every xcorr value are signed). We use its generalization,
Yeo-Johnson (sklearn's PowerTransformer), which extends the same idea to
the full real line. The transform is fit on training features only (no
target leakage, no test-set fitting) and reused unchanged on the test
features, matching the "any learned preprocessing must be fit using
training data only" rule. `standardize=True` also folds in the final
zero-mean/unit-variance scaling, so no separate StandardScaler is needed.
"""
import sys
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v5.py train.csv test.csv predictions.txt")
        sys.exit(1)
