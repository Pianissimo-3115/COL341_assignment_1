#!/usr/bin/env python3
"""Part (c) variant v10: part_3_v7.py's correlation-free feature set
(pulse morphology, exponential recency-weighting, motion/quality
indicators) with ElasticNetCV in place of RidgeCV.

Usage:
    python3 part_3_v10.py train.csv test.csv predictions.txt
...
Everything except the final model is identical to part_3_v7.py -- see that
file's docstring for the feature-family rationale. ElasticNetCV's L1
component can zero out redundant/uninformative columns (e.g. among the
EWMA half-lives or the block-trend slopes), which RidgeCV cannot do, at
the cost of a much more expensive l1_ratio x alpha coordinate-descent
search compared to RidgeCV's closed-form per-alpha solve.
"""
import sys
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 part_3_v10.py train.csv test.csv predictions.txt")
        sys.exit(1)
